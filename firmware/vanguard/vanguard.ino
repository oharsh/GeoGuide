/*
 * Vanguard -- ESP32 firmware for the line-following robot.
 *
 * The robot follows a black line using five IR sensors and takes its
 * turn-by-turn instructions from the host over a TCP socket. Each character of
 * the route string is one instruction consumed at a node:
 *
 *   I - leaving the start box       Q - returning to the start box
 *   S - carry straight on           L / R - turn left / right
 *   T / F - the two possible orientations of the robot's head at the end of a
 *           leg, used to tell it which way to be facing when it arrives
 *
 * It talks back to the host with three messages: ALERT (closing on the
 * destination, watch for me), SEND (ready for the next route) and HOME (on the
 * final approach). The host replies STOP when the overhead camera sees the
 * robot sitting on the target square.
 *
 * WiFi credentials live in secrets.h -- copy secrets.example.h to secrets.h and
 * fill in your own before flashing.
 */

#include <WiFi.h>
#include <string.h>

// WiFi credentials and host address -- see secrets.example.h
#include "secrets.h"
...
void setup()
{
  ...
  // Request the first route, to the highest-priority event
