...
*
*
* Example Call:    arena_image(arena_path)
'''
def arena_image():   
    while True:
...
def priority(a):
    if a[1] == "fire":
        return 1
    if a[1] == "destroyed_buildings":
        return 2
    if a[1] == "human_aid_rehabilitation":
        return 3
    if a[1] == "military_vehicles":
        return 4
    if a[1] == "combat":
        return 5
    if a[1] == " ":
...
def event_locator(arena): 
    ...
    _, thresholded = cv.threshold(gray, 185, 255, cv.THRESH_BINARY)
...
def main():
    ...
    identified_labels = {chr(i+65): detected_list[i] for i in range(0, len(detected_list)) if detected_list[i] != " "}
...
#   event_configuration = task_4a_return()  
#   with open("/home/deadmonk/Desktop/eyrc23_GG_1667/Tasks/Task5/event/events.pickle", "wb") as f :
#     pickle.dump(event_configuration, f)
#   f.close()

# #   dkey = identified_labels.keys()
# #   dkey = sorted(dkey)
# #   identified_labels = {i : identified_labels[i] for i in dkey}
#   event_configuration = dict(sorted(event_configuration.items(), key= lambda a: a[0]))
#   print(event_configuration)
